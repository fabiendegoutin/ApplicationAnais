...
model = genai.GenerativeModel(
    model_name='models/gemini-1.5-flash', # Utilisation de 1.5 Flash, plus stable pour le quota gratuit
    system_instruction="Tu es le coach d'Anaïs. Pose uniquement des QCM (A, B, C) basés sur le texte fourni. Saute des lignes entre les choix."
)
...
# --- 1. LECTURE DU COURS (UN SEUL APPEL API) ---
    ...
    if photo and st.button("🚀 LANCER LE QUIZZ"):
        try:
            with st.spinner("Analyse du cours..."):
                ...
                img.thumbnail((800, 800)) # Réduction de taille pour économiser les tokens
                res = model.generate_content(["Extrais le texte de ce cours.", img])
                st.session_state.cours_texte = res.text
                
                # Première question
                q = model.generate_content(f"Cours: {st.session_state.cours_texte}. Pose une question QCM courte.")
                st.session_state.messages.insert(0, {"role": "assistant", "content": q.text})
                st.rerun()
        except exceptions.ResourceExhausted:
            st.error("⚠️ Le coach est un peu fatigué. Attends 15 secondes et reclique sur Lancer ! 😊")
...
# --- 2. LE QUIZZ ---
    ...
    if choix:
        try:
            ...
            st.warning("⏱️ Trop rapide ! Attends 10 secondes avant de répondre à nouveau.")
            st.session_state.nb_q -= 1 # Annule le compteur pour cette tentative
    ...
    for i, msg in enumerate(st.session_state.messages):
        with st.chat_message(msg["role"], avatar="🌈" if msg["role"]=="assistant" else "⭐"):
            ...
            if msg["role"] == "assistant" and st.button("🔊 Écouter", key=f"s_{i}"):
